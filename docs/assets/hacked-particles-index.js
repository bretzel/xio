...
var ParticlesManager$$1 = (function () {
  ...
  ParticlesManager$$1.prototype.particlesCreate = function () {
    ...
    if (this._params.particles.statics != undefined) {
      console.log(`Have ${this._params.particles.statics} as  static particles:`)
      if (this._params.particles.statics.length) {

        this._params.particles.number.value += this._params.particles.statics.length;

        var dir = this._params.particles.move.direction;
        var spd = this._params.particles.move.speed;
        var sz = this._params.particles.size.value;
        var ran = this._params.particles.size.random;

        this._params.particles.move.direction = '';
        this._params.particles.move.speed = 0;
        this._params.particles.size.value = 1;
        this._params.particles.size.ranom = false;

        for (var /** @type {?} */ vsp = 0; vsp != this._params.particles.statics.length; vsp++) {
          var /** @type {?} */ El = document.getElementById(this._params.particles.statics[vsp]);
          if (!El) break;
          var /** @type {?} */ pxy = {
          ...
          this._params.particles.array.push(
            new Particle$$1(this._canvasParams, this._params, this._tmpParams, color, opacity.value, pxy)
          );
          console.log(`Particle ${this._params.particles.statics[vsp]} created at (${pxy.x},${pxy.y})`);
        }
        this._params.particles.move.direction = dir;
        this._params.particles.move.speed = spd;
        this._params.particles.size.value = sz;
        this._params.particles.size.random = ran;
  ...
  ParticlesManager$$1.prototype._particlesUpdate = function () {
    ...
    this._params.particles.array.forEach(function (particle, i) {
      var _statics_begin = 0;
      if (_this._params.particles.statics != undefined) {
        _statics_begin = _this._params.particles.array.length - _this._params.particles.statics.length;
      }
      if (i < _statics_begin) {
